...
def generate_gaussian_target(polys, h, w):
    ...
    geometry_map = np.zeros((h, w, 4), dtype=np.float32)
    ...
    for poly_idx, poly in enumerate(polys):
        ...
        for y, x in xy_in_poly:
            ...
            geometry_map[y, x, 0] = top_x
            geometry_map[y, x, 1] = top_y
            geometry_map[y, x, 2] = bot_x
            geometry_map[y, x, 3] = bot_y
        ...
        stride = 1
        top_pts = topside_pts[stride:-stride]
        bot_pts = bottomside_pts[stride:-stride][::-1]
        center_pts = []
        radius = []
        ...
        try:
            ...
            char_box = np.array([top_pts[0], top_pts[R-1], bot_pts[R-1], bot_pts[0]]).astype(np.float32)
        ...
        for i, Rk in enumerate(radius[R:-R]):
    
            cpts = center_pts[R + i]
            cx, cy = list(map(int, cpts))
            cx1, cy1 = list(map(int, center_pts[R + i + 1]))
...
class IC15Loader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        geometry_map = np.zeros((h, w, 4), dtype='float32')
        ...
        if bboxes.shape[0] > 0:
            ...
            h, w = img.shape[:2]
            t1 = time.time()
            gt_text, geometry_map = generate_gaussian_target(bboxes, h, w)
            dur = time.time() - t1
        ...
        return img, probability_map, training_mask, np.array(ori_img), geometry_map
