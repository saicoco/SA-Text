...
def generate_gaussian_target(polys, h, w):
    ...
    for poly in polys:
        ...
        # long_poly1 = poly[e1[0]]
        # long_poly2 = poly[e2[0]]
        ...
        stride = 1
        top_pts = topside_pts[0::stride]
        bot_pts = bottomside_pts[0::stride][::-1]
        center_pts = []
        radius = []
...
class IC15Loader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if bboxes.shape[0] > 0:
            ...
            h, w = img.shape[:2]
            t1 = time.time()
            gt_text = generate_gaussian_target(bboxes, h, w)
            dur = time.time() - t1
        ...
        return img, probability_map, training_mask, np.array(ori_img)
...
if __name__ == "__main__":
    ...
    for item in ic15dataset:
        img, pb_map, train_mask, ori_img = item
        ...
        att_im = cv2.addWeighted(seg_map_3c.astype(np.uint8), 0.9, np.array(ori_img)[:, :,::-1], 0.2, 0.0)
        # save_img=np.concatenate((np.array(ori_img),att_im),1)
        region = np.where(seg_map_3c[:, :, 0] > int(0.05 * 255), np.ones_like(seg_map_3c[:, :, 0]) * 255, np.zeros_like(seg_map_3c[:, :, 0]))
